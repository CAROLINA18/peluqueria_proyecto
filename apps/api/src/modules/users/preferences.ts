...
preferencesRouter.patch('/me/preferences', authenticate, async (req, res) => {
  ...
  res.json({ accessToken: createAccessToken(user), user: { id: user.id, name: user.name, username: user.username, role: user.role, preferredLocale: input.locale, mustChangePassword: user.mustChangePassword } });
});
