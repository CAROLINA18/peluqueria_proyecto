...
- [ ] 0.1 Registrar EUR, `Europe/Brussels`, `Lina Quirama Beauty Salon` y los dos archivos de `logos/` como decisiones confirmadas; la dirección queda opcional. `[PRODUCT, REQ-REPORT-001, REQ-EXPORT-001, REQ-BRAND-001]`
- [ ] 0.2 Confirmar si los bonos requieren saldo/código o continúan como medios de pago administrables. `[REQ-CAT-001, REQ-SALE-001]`
- [ ] 0.3 Hacer wireframe responsive del flujo de venta dinámica y validarlo con un usuario real. `[REQ-NFR-001]`
- [ ] 0.4 Registrar decisiones resultantes y cerrar preguntas abiertas. `[STEERING]`
...
- [ ] 1.2 Inicializar Angular standalone/strict y Express/TypeScript strict con health endpoints. `[REQ-NFR-005]`
- [ ] 1.3 Configurar ESLint, Prettier, Vitest, Playwright, scripts raíz y hooks opcionales. `[REQ-NFR-005]`
- [ ] 1.4 Crear `.env.example` y validación tipada con fallo temprano. `[REQ-NFR-004]`
- [ ] 1.5 Crear Dockerfile multi-stage, `compose.yaml`, healthchecks y volumen MySQL. `[REQ-NFR-004]`
- [ ] 1.6 Implementar scripts setup/dev/build/test/migrate/seed/up/down/backup/restore para PowerShell y Bash. `[REQ-NFR-004]`
- [ ] 1.7 Escribir README “inicio en 5 minutos” y runbook de backup/restore. `[REQ-NFR-004]`
...
- [ ] 2.1 Modelar Prisma completo, constraints, índices y primera migración. `[DESIGN §4]`
- [ ] 2.2 Crear seed idempotente con admin de desarrollo, categorías, servicios de muestra y medios de pago administrables. `[REQ-CAT-001, REQ-NFR-004]`
- [ ] 2.3 Definir contratos Zod/DTO y OpenAPI 3.1 para auth, usuarios, servicios, ventas, reportes y errores. `[REQ-NFR-005]`
- [ ] 2.4 Implementar middleware de problem details, request id, logs redactados, Helmet, CORS dev y rate limit. `[REQ-NFR-003]`
- [ ] 2.5 Implementar aritmética decimal, fechas de negocio y políticas RBAC como módulos probados. `[REQ-SALE-001, REQ-REPORT-001]`
- [ ] 2.6 Implementar auditoría append-only y helper transaccional. `[REQ-AUDIT-001]`
- [ ] 2.7 Probar constraints, transacciones, zona horaria, dinero y capacidades con MySQL. `[REQ-NFR-005]`
- [ ] 2.8 Crear infraestructura i18n, catálogos base `es`/`en`, verificación de claves y preferencia `preferredLocale`. `[REQ-I18N-001]`

**Checkpoint:** modelo migrable y políticas críticas cubiertas antes de exponer CRUD.

## Fase 3 — Autenticación y usuarios

- [ ] 3.1 Implementar Argon2id, login genérico, access token corto, refresh rotatorio y logout. `[REQ-AUTH-001]`
- [ ] 3.2 Implementar revocación por desactivación/cambio de rol, bloqueo progresivo y auditoría. `[REQ-AUTH-001]`
- [ ] 3.3 Implementar CRUD de usuarios, password temporal y protección del último admin. `[REQ-USER-001]`
- [ ] 3.4 Crear sesión frontend, interceptor de refresh único, guards y pantalla login accesible. `[REQ-AUTH-001, REQ-NFR-001]`
- [ ] 3.5 Crear UI de usuarios solo admin con estados completos y confirmaciones específicas. `[REQ-USER-001]`
- [ ] 3.6 Ejecutar pruebas negativas de sesión, CSRF, enumeración, rol e IDOR. `[REQ-NFR-003]`
- [ ] 3.7 Implementar `/users/me/preferences`, selector ES/EN y restauración del idioma al autenticar. `[REQ-I18N-001]`

**Checkpoint:** los tres perfiles autentican y la matriz base es imposible de eludir por API.

## Fase 4 — Catálogos administrativos

- [ ] 4.1 Implementar repositorios/casos de uso/endpoints de servicios, categorías y medios de pago con normalización, orden y baja lógica. `[REQ-SVC-001, REQ-CAT-001]`
...
- [ ] 5.1 Implementar creación transaccional, snapshots, cálculo servidor, pagos divididos e idempotencia. `[REQ-SALE-001]`
- [ ] 5.2 Aplicar fecha actual y ownership del asistente dentro de consultas/casos de uso. `[REQ-SALE-002]`
- [ ] 5.3 Aplicar permisos del senior y pruebas negativas. `[REQ-SALE-003]`
- [ ] 5.4 Implementar listado/detalle paginado y filtros con scope por rol. `[REQ-SALE-002, REQ-SALE-004]`
- [ ] 5.5 Implementar edición optimista y anulación admin con motivo/auditoría. `[REQ-SALE-004]`
- [ ] 5.6 Crear formulario Angular dinámico de líneas y pagos, selector de medio, precio editable, nota obligatoria al cambiarlo y resumen fijo. `[REQ-SALE-001, REQ-CAT-001, REQ-NFR-001]`
...
- [ ] 6.1 Implementar periodos día/semana/mes/año/rango para admin y restricción estricta día/mes para senior, con consultas agregadas indexadas. `[REQ-REPORT-001]`
- [ ] 6.2 Implementar DTO `ReportSnapshot` y reconciliación de líneas/pagos/total. `[REQ-REPORT-001]`
- [ ] 6.3 Crear pantalla con filtros, KPIs, desgloses, tabla accesible y estados vacíos. `[REQ-REPORT-001, REQ-NFR-001]`
- [ ] 6.4 Implementar PDF A4 con sello oficial y cabecera `Lina Quirama Beauty Salon`, paginación y datos del periodo. `[REQ-EXPORT-001, REQ-BRAND-001]`
- [ ] 6.5 Implementar XLSX de cuatro hojas, formatos y mitigación de formula injection. `[REQ-EXPORT-001]`
- [ ] 6.6 Aplicar autorización/rate limit/auditoría a ambas exportaciones. `[REQ-EXPORT-001]`
- [ ] 6.7 Probar igualdad UI/PDF/XLSX con datos, sin datos, anulaciones y bordes de periodo. `[REQ-REPORT-001, REQ-EXPORT-001]`
- [ ] 6.8 Traducir PDF/XLSX según preferencia y probar títulos, hojas, fechas e importes en `es` y `en`. `[REQ-I18N-001, REQ-EXPORT-001]`
...
- [ ] 7.1 Crear dashboard específico por perfil. `[REQ-DASH-001]`
- [ ] 7.2 Implementar consulta y UI de auditoría para admin. `[REQ-AUDIT-001]`
- [ ] 7.3 Derivar y aplicar tokens visuales de los logos, crear `BrandLogoComponent` y completar responsive, skeletons, vacíos y errores. `[REQ-BRAND-001, REQ-NFR-001]`
- [ ] 7.3.1 Validar contraste de todas las parejas de texto/fondo y estados interactivos antes de aprobar el tema. `[REQ-BRAND-001, REQ-NFR-001]`
...
- [ ] 7.6 Revisar textos en español, localización de EUR/fechas y mensajes de negocio. `[UX]`
- [ ] 7.7 Completar inglés, revisar expansión de textos, fallback, cambio en caliente y ausencia de claves visibles. `[REQ-I18N-001]`
